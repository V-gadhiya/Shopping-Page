...
const ProductDetails = () => {
  ...
  const [zoomImageSet, setZoomImageSet] = useState(false);
  ...
  const fetchProductDetails = async () => {
    ...
    const responce = await fetch(SummaryApi.productDetails.url, {
      ...
      body: JSON.stringify({
        _id: params.id, // Assuming your route parameter is named "id"
      }),
    });
    setLoading(false);
      ...
      <div className="min-h-[200px] flex flex-col lg:flex-row gap-4">
        <div className="h-96 flex flex-col lg:flex-row-reverse gap-4">
          ...
          <div className="h-full">
            {loading ? (
              <div className="flex gap-2 lg:flex-col overflow-scroll scrollar-none h-full">
              ...
              <div className="flex gap-2 lg:flex-col overflow-scroll scrollar-none h-full">
                {data?.productImage?.map((imgURL, index) => {
                  return (
                    <div
                      key={imgURL}
                      className="h-20 w-20 p-1 bg-slate-200 rounded"
                    >
                      <img
                        src={imgURL}
                        alt=""
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-1">
            <p className="bg-red-200 text-red-600 px-2 rounded-full inline-block">
              {data?.brandName}
            </p>
            ...
            <div className="flex gap-3 items-center my-2">
              <button
                ...
                onClick={(e) => {
                  handleByeProduct(e, data?._id);
                }}
              >
                Buy
              </button>
              <button
                className="border-2 border-red-600 rounded px-3 py-1 min-w-[100px] text-white font-medium bg-red-600 hover:bg-white hover:text-red-600"
                onClick={(e) => {
